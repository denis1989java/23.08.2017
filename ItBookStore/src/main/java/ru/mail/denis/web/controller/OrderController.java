...
public class OrderController {
    ...
    public ModelAndView showAllOrders(@PathVariable int page, @RequestParam(value = "orderId", required = false) String orderId) {
        ...
        ModelAndView modelAndView = new ModelAndView("admins/allOrders");
        if (orderId != null) {
            List<OrdersBooksDTO> ordersBooksDTO = orderservice.getOrderBooksDTOByOrderId(Integer.valueOf(orderId));
            modelAndView.addObject("ordersBooksDTO", ordersBooksDTO);
            modelAndView.addObject("orderID", orderId);
    ...
    public String showChangeOrder(@RequestParam("orderId") String orderId, Model model) {
        ...
        for (int i = 0; i < bookTimesDTOS.size(); i++) {
            BigDecimal price = bookTimesDTOS.get(i).getBookPrice();
            Integer quantity = bookTimesDTOS.get(i).getBookQuantity();
            summ = summ.add(price.multiply(new BigDecimal(quantity))) ;
        }
        model.addAttribute("orderDTO", orderDTO);
        model.addAttribute("summ", summ);
        model.addAttribute("OrderBookTimesDTO", bookTimesDTOS);
